...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['update'])) {
        ...
        // Update admin
        $update_product = $conn->prepare("UPDATE `staff` SET name = ?, email = ?, password = ?, profile = ? WHERE id = ?");
        ...
        if (!empty($profile)) {
            $select_image = $conn->prepare("SELECT * FROM `staff` WHERE profile = ?");
            ...
            if ($image_size > 2000000) {
                ...
                $update_image = $conn->prepare("UPDATE `staff` SET profile = ? WHERE id = ?");
    ...
    if (isset($_POST['delete'])) {
        ...
        $delete_image = $conn->prepare("SELECT * FROM `staff` WHERE id = ? ");
        ...
        $delete_product = $conn->prepare("DELETE FROM `staff` WHERE id = ?");
...
<body>
    ...
    <div class="banner-login">
        ...
        <div class="title2">
            <a href="staff_dashboard.php">Dashboard</a><span> / Edit Profile</span>
        </div>
        <section class="edit-post">
            <h1 class="heading">Edit Profile</h1>
            <?php
                $select_product = $conn->prepare("SELECT * FROM `staff` WHERE id = ?");
             ...
             <div class="form-container">
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="flex-btn">
                        ...
                        <a href="staff_dashboard.php" class="btn">Go back</a>
